# import ConfigParser
#~ Ty a different angle or shoot away from the tree. Also, if you know how to use manual mode, you can choose a low ISO (100) and 1/60 Shutter Speed. 
#~ The lower the ISO, the more light can come in, also, 100 ISO is good for shooting in daylight. 
#~ Whenever I shoot in daylight, I just use 5.6 Aperture and I just change all the other settings. Anyway, hope this helps.
# camera ayarlarından bahsediyo gibi:
#~ https://www.quora.com/Why-does-my-skin-color-seem-to-change-in-pictures
...
#~ https://www.chiefdelphi.com/t/changing-usb-camera-settings-on-raspberry-pi/156882/3
import cv2
import numpy as np
import time
import gor_class
...
class BalloonConfig:
	...
	def save_callback(self,x):
		if x ==1:
			with open('balloon_config_file_webcam.cnf','w') as dosya:
	...
	def set_initials_of_sliders(self):
		...
		cv2.setTrackbarPos('Save', 'Bars', 0)		#255
	...
	def main(self):
		...
		while True:
			...
			#~ self.mask_ic = imutils.rotate(self.mask_ic,angle=180)
			...
			#~ print("horizontal_pixels_of_balloon: ",self.horizontal_pixels_of_balloon_ic)
			mask_rescaled = self.rescale_frame(self.mask_ic, percent=per)
			frame_rescaled = self.rescale_frame(self.frame, percent=per)
			#~ frame_rescaled = imutils.rotate(frame_rescaled,angle=180)
